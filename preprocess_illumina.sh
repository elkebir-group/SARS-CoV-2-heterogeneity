...
do
    ...
    then
		...
		bwa mem -t 4 reference.fasta $f > sam/$(basename $f .fastq).sam
		counter=$(( $counter + 1 ))
    fi
done
...
do
    ...
    then
        ...
        samtools sort sam/${seqName}.sam -o sorted_sam/${seqName}.sorted.sam
		counter=$(( $counter + 1 ))
    fi
done
...
do
    ...
    then
        ...
        samtools view -S -b sorted_sam/${seqName}.sorted.sam > sorted_bam/${seqName}.sorted.bam
		counter=$(( $counter + 1 ))
    fi
done
...
do
	...
	then
		...
		gatk --java-options \"-Djava.io.tmpdir=/scratch/tmp -Xmx12G\" MarkDuplicates -I sorted_bam/${seqName}.sorted.bam -O sorted_dedup_bam/${seqName}.sorted.dedup.bam -M sorted_dedup_bam/${seqName}.dedup.metrics.txt
		counter=$(( $counter + 1 ))
	fi
done
...
do
	...
	then
		...
		samtools index sorted_dedup_bam/$seqName.sorted.dedup.bam
		counter=$(( $counter + 1 ))
	fi
done
